...
Il ne nous reste plus qu'à sauvegarder notre fichier nouvellement créé. Nous voulons le placer à la racine de notre compte utilisateur, c'est à dire l'enregistrer sous \hlred{\Verb|\textasciitilde{}/.vimrc|}. Sous Mac OS X et Linux, \hlred{\Verb|\textasciitilde{}|} désigne le répertoire d'accueil de l'utilisateur courant. Attention, les fichiers commençant par un \hlred{.} sont des fichiers cachés sous Linux et Mac OS X, ne vous étonnez donc pas de ne pas le voir par défaut dans votre navigateur de fichiers.
...
J'ai mis en ligne ce fichier de configuration directement sur \emph{Github}. Vous pouvez le télécharger ou le copier directement à partir d'ici : \url{http://www.vimebook.com/link/fr/firstconfig}. Il devrait aussi faire partie du package que vous avez téléchargé.
