...
\newthought{Passons aux choses sérieuses}, c'est à dire comment rendre \vim un tant soit peu utilisable. Nous allons donc éditer le fichier de configuration par défaut \emph{.vimrc} en y plaçant des valeurs que toute personne normalement constituée souhaiterait y voir figurer.

\begin{listing}[H]
\begin{fullwidth}
    \begin{minted}[bgcolor=bg, fontsize=\footnotesize]{vim}
        " VIM Configuration - Vincent Jousse
        " Annule la compatibilte avec l'ancetre Vi : totalement indispensable
        set nocompatible

        " -- Affichage
        set title                         " Mets a jour le titre de votre fenetre ou de votre
                                          " terminal
        set number                        " Affiche le numero des lignes
        set ruler                         " Affiche la position actuelle du curseur
        set wrap                          " Affiche les lignes trop longues sur plusieurs
                                          " lignes
        "set nowrap                       " Ne coupe pas les lignes trop longues a
                                          " l'affichage
